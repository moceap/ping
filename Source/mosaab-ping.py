...
system("title "+"Mosaab Ping Tool v0.0.1")
print("Welcome to Mosaab Ping Tool\n")
repeat = True
def sethost():
    global host
...
def result(income):
    if(income == 0):
        print(host +" is UP and PONG")
    else:
        print(host +" is DOWN and CANT be pong")

while(repeat == True):
    choice = input("Select A Method For Pinging (1-3):\n"+
                   "1) System Ping\n"+
                   "2) Pure Python Ping\n")
    if (choice == "1" or choice == "2" or choice == "3"):
        repeat = False
if (choice == "1"):
    print("\nSystem Ping .. Selected.   \n")
    sethost()
    import os
    import platform
    param = '-n' if platform.system().lower()=='windows' else '-c'
    result(os.system("ping " + param + " 1 " + host))

##### INCLUDING PURE PYTHON PINGINIG    
"""
...
def multi_ping_query(hosts, timeout=1, step=512, ignore_errors=False):
    ...
    return results
##### END INCLUDING PURE PYTHON PINGINIG    

if (choice == "2"):
    print("\nPure Python Ping .. Selected.   \n")
    sethost()
    try:
        res = float(ping(host))
        print()
        print("PONG in " + "%.0f" % res + " ms")
    except:
        print(ping(host))
input("\n ENTER to EXIT :)")
